...
import com.gift.compass.global.AppUtils;
import com.gift.compass.global.solar.SunriseSunsetCalculator;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initializeView() {
        ...
        mMainView.setBackground(
            ContextCompat.getDrawable(getApplicationContext(), R.drawable.bg_default));
        ...
        if (bundle != null && bundle.containsKey(BACKGROUND) &&
            bundle.containsKey(GREETING_MESSAGE) && bundle.containsKey(LOCATION)) {
            ...
            if (background != -1 && greetingMessage != -1) {
                mMainView.setBackground(
                    ContextCompat.getDrawable(getApplicationContext(), background));
                ...
                if (location != null) {
                    ...
                    mPositionTextView.setText(
                        AppUtils.convert(location.getLatitude(), location.getLongitude()));
                } else {
        ...
        ZcoupSDK.getBannerAd(this, "72734208", AdSize.AD_SIZE_320X50, new AdEventListener() {
            @Override
    ...
    private void updateDayState(Location location) {
        ...
        com.gift.compass.global.solar.Location location1 =
            new com.gift.compass.global.solar.Location(location.getLatitude(),
                location.getLongitude());
        SunriseSunsetCalculator sunriseSunsetCalculator = new SunriseSunsetCalculator(location1,
            Calendar.getInstance().getTimeZone());

        Date sunrise = sunriseSunsetCalculator.getOfficialSunriseCalendarForDate(
            Calendar.getInstance()).getTime();
        Date sunset = sunriseSunsetCalculator.getOfficialSunsetCalendarForDate(
            Calendar.getInstance()).getTime();
        ...
        if (current.after(midNight)) {
            mMainView.setBackground(
                ContextCompat.getDrawable(getApplicationContext(), R.drawable.bg_night));
            mGreetingTextView.setText(R.string.night_greeting);

            if (current.after(sunrise)) {
                mMainView.setBackground(
                    ContextCompat.getDrawable(getApplicationContext(), R.drawable.bg_morning));
                mGreetingTextView.setText(R.string.morning_greeting);

                if (current.after(noon)) {
                    mMainView.setBackground(
                        ContextCompat.getDrawable(getApplicationContext(), R.drawable.bg_evening));
                    mGreetingTextView.setText(R.string.afternoon_greeting);

                    if (current.after(sunset)) {
                        mMainView.setBackground(ContextCompat.getDrawable(getApplicationContext(),
                            R.drawable.bg_night));
                        mGreetingTextView.setText(R.string.night_greeting);
                    }
                }
