...
public class Compass extends RelativeLayout implements SensorEventListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        SensorManager mSensorManager = (SensorManager) getContext().getSystemService(
            SENSOR_SERVICE);
        ...
        TypedArray typedArray = context.getTheme()
            .obtainStyledAttributes(attrs, R.styleable.Compass, 0, 0);

        if (typedArray != null) {

            mShowBorder = typedArray.getBoolean(R.styleable.Compass_show_border,
                DEFAULT_SHOW_BORDER);
            mBorderColor = typedArray.getColor(R.styleable.Compass_border_color,
            ...
            mShowOrientationLabels = typedArray.getBoolean(
                R.styleable.Compass_show_orientation_labels, DEFAULT_SHOW_ORIENTATION_LABEL);
            mOrientationLabelsColor = typedArray.getColor(
                R.styleable.Compass_orientation_labels_color, DEFAULT_ORIENTATION_LABEL_COLOR);

            mDegreeValueColor = typedArray.getColor(R.styleable.Compass_degree_value_color,
                Color.BLACK);
            mShowDegreeValue = typedArray.getBoolean(R.styleable.Compass_show_degree_value,
                DEFAULT_SHOW_DEGREE_VALUE);

            mDegreesStep = typedArray.getInt(R.styleable.Compass_degrees_step,
                DEFAULT_DEGREES_STEP);
        ...
        compassSkeleton.getViewTreeObserver()
            .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    compassSkeleton.setPadding(needlePadding, needlePadding, needlePadding,
                        needlePadding);
    ...
    private void updateTextDirection(double degree) {
        ...
        String value;
        if (deg > 0 && deg <= 90) {
            value = String.format("%s%s North East", String.valueOf(decimalFormat.format(deg)), DEGREE);
        } else if (deg > 90 && deg <= 180) {
            value = String.format("%s%s South East", String.valueOf(decimalFormat.format(deg)), DEGREE);
        } else if (deg > 180 && deg <= 270) {
            value = String.format("%s%s South West", String.valueOf(decimalFormat.format(deg)), DEGREE);
        } else {
            value = String.format("%s%s North West", String.valueOf(decimalFormat.format(deg)), DEGREE);
        }
